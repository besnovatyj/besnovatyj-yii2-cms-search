...
ActiveRecord). Кнопка пересборки есть и в админке — раздел «Поисковый индекс», а также на плитке
главной панели (модуль `yii2-cms-dashboard`): плитка показывает дату сборки, ядро, счётчики
«в индексе / в каталоге» и собирает индекс AJAX-запросом, не уводя с панели. Тот же эндпойнт
`/Search/backend/index/rebuild` отвечает JSON'ом на AJAX и редиректом на обычный POST.
